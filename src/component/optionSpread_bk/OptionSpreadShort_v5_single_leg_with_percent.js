  ...
  const checkAndSetStopLoss = () => {
    if (niftyValue.niftyPos === "long") {
      ...
      let slPeStockTriggerPrice =
        ((stopLoss * tmpBufferPeStockClose) / 100) * 50;
      optionSlTrigger.value = twoDigitDecimal(-slPeStockTriggerPrice);
    } else {
      let ceStockClose =
        completeData[candleTime.value - 1][optionValue.value].stockClose;
      ...
      let slCeStockTriggerPrice =
        ((stopLoss * tmpBufferCeStockClose) / 100) * 50;
      optionSlTrigger.value = twoDigitDecimal(-slCeStockTriggerPrice);
    }
