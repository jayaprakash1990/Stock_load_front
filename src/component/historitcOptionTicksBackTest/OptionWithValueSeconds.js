...
const OptionWithValueSeconds = () => {
  ...
  const targetBooking = 600;
  const stopLossBooking = -500;

  const strikeMoneyRange = 50;
  ...
  useEffect(() => {
    if (niftyValue && niftyValue.last_price) {
      ...
      let tmpCeValue = {
        label: "NIFTYWK" + (strike - strikeMoneyRange) + "CE",
        value: "NIFTYWK" + (strike - strikeMoneyRange) + "CE",
      };
      let tmpPeVale = {
        label: "NIFTYWK" + (strike + strikeMoneyRange) + "PE",
        value: "NIFTYWK" + (strike + strikeMoneyRange) + "PE",
      };
  ...
  const calculateValue = (tDayEndMax) => {
    ...
    for (let i = 0; i < completeData.length; i++) {
      if (
        ...
        let isStopLossHit = "";
        if (totValue > targetBooking) {
          isStopLossHit = "TARGET";
        } else if (totValue < stopLossBooking) {
          isStopLossHit = "STOPLOSS";
        }
...
export default OptionWithValueSeconds;
